...
// 从ILI9341读出的数据为GBR格式，而我们写入的时候为RGB格式。
// 通过该函数转换
// c:GBR格式的颜色值
// 返回值：RGB格式的颜色值
...
u16 LCD_ReadPoint(u16 x, u16 y)
{
    ...
    LCD_SetCursor(x, y);
    LCD_WR_REG(0X2E);           // ILI9341 发送读GRAM指令

    r = LCD_RD_DATA();          // 假读(Dummy Read)
    r = LCD_RD_DATA();          // 实际坐标颜色
    b = LCD_RD_DATA();
    
    g = r & 0XFF;               // 对于ILI9341, 第一次读取的是RG的值, R在前, G在后, 各占8位
    g <<= 8;
...
{
    LCD_WR_REG(0X29);       // ILI9341 开启显示指令
}

// LCD关闭显示
void LCD_DisplayOff(void)
{
    LCD_WR_REG(0X28);       // ILI9341 关闭显示指令
}
...
// 注意:一般设置为L2R_U2D即可,如果设置为其他扫描方式,可能导致显示不正常.
...
void LCD_Init(void)
{
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB | RCC_APB2Periph_GPIOD | RCC_APB2Periph_GPIOE | RCC_APB2Periph_GPIOG, ENABLE); //使能PORTB,D,E,G复用
